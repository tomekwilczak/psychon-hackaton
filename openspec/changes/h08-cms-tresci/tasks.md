...
- [ ] 2.1 Zarejestrować trasy kursów w `backend/routes/api/h08.php` za wczesnym
...
- [ ] 2.2 Utworzyć `App\Http\Requests\H08\StoreCourseRequest` i `UpdateCourseRequest`
      ...
      `validation_failed`.
- [ ] 2.3 Utworzyć `App\Services\H08\CourseWriter` z `create` / `update` / `delete`,
...
- [ ] 2.4 Zaimplementować w `CourseWriter` regułę „publikacja bez lekcji" — sprawdzenie
      **po** złożeniu stanu docelowego, przed zapisem, `ApiException(422,
      ...
      zmienia rekordu.
- [ ] 2.5 Zaimplementować w `CourseWriter` regułę „usunięcie prerekwizytu" — 422
...
- [ ] 2.6 Utworzyć `App\Http\Controllers\Api\V1\Admin\CourseCatalogAdminController`
...
- [ ] 2.7 Utworzyć `App\Http\Resources\H08\AdminCourseResource` z polami `{id, title,
...
- [ ] 2.8 Utworzyć `backend/tests/Feature/H08/AdminCourseTest.php` w konwencji repo
...
- [ ] 2.9 Bramki fazy: `docker compose exec app php artisan test --filter=H08`, pełny
...
- [ ] 3.1 Dopisać trasy lekcji do grupy z 2.1: `GET/POST /admin/courses/{course}/lessons`,
...
- [ ] 3.2 Utworzyć `App\Http\Requests\H08\StoreLessonRequest` i `UpdateLessonRequest`
...
- [ ] 3.3 Utworzyć `App\Services\H08\LessonWriter` — zapis, numeracja domyślna i audyt
...
- [ ] 3.4 Zaimplementować soft delete lekcji przez `SoftDeletes` na modelu — **żaden**
...
- [ ] 3.5 Utworzyć `App\Http\Controllers\Api\V1\Admin\LessonAdminController` (`index`
...
- [ ] 3.6 Utworzyć `App\Http\Resources\H08\AdminLessonResource` z polami `{id, course_id,
...
- [ ] 3.7 Utworzyć `backend/tests/Feature/H08/AdminLessonTest.php` — kryterium ★2
...
- [ ] 3.8 Bramki fazy: `--filter=H08`, pełna suita, `./vendor/bin/pint`; weryfikacja:
      wszystkie zielone.
...
- [ ] 4.1 Dopisać trasy kolejności: `PATCH /admin/courses/{course}/lessons/reorder`,
...
- [ ] 4.2 Utworzyć `App\Services\H08\SequenceReorderer` z `reorderLessons(Course, array)`
...
- [ ] 4.3 Dodać w `SequenceReorderer` walidację pełnej permutacji (brak / nadmiar /
...
- [ ] 4.4 Utworzyć `App\Http\Requests\H08\ReorderLessonsRequest` i
...
- [ ] 4.5 Utworzyć `App\Services\H08\ReorderImpactPreview::for(array $courseIds): array`
...
- [ ] 4.6 Zawęzić zakres podglądu do kont `role ∈ {volunteer, student}` ze
...
- [ ] 4.7 Zwrócić z podglądu pozycje `{user_id, first_name, last_name, course_id,
...
- [ ] 4.8 Utworzyć `backend/tests/Feature/H08/ReorderTest.php` — kryterium 3: liczba
...
- [ ] 4.9 Bramki fazy: `--filter=H08`, pełna suita, `./vendor/bin/pint`; weryfikacja:
      wszystkie zielone.
...
- [ ] 5.1 Utworzyć `frontend/lib/slots/admin-courses.ts` na wzór
...
- [ ] 5.2 Utworzyć `frontend/lib/menu/admin/h08-kursy.ts`
      ...
      20, 25).
- [ ] 5.3 Utworzyć `frontend/lib/h08/types.ts` z `AdminCourse`, `AdminLesson`,
...
- [ ] 5.4 Utworzyć `frontend/app/(administracja)/admin/kursy/page.tsx` —
...
- [ ] 5.5 Utworzyć `frontend/app/(administracja)/admin/kursy/[id]/page.tsx` — formularz
      ...
      surowy błąd.
- [ ] 5.6 Wyrenderować regiony slotów przez `slotsForRegion(...)` na karcie kursu — na
      koniec fazy nie renderują jeszcze nic; weryfikacja: brak błędów przy pustej
      tablicy slotów, `npm run build` przechodzi.
- [ ] 5.7 Utworzyć `frontend/components/h08/ReorderConfirmModal.tsx` na natywnym
...
- [ ] 5.8 Bramki fazy: `npm run lint -- --fix` i `npm run build`; weryfikacja: obie
      czyste.
...
- [ ] 6.1 Nanieść rozstrzygnięcia strażnika K1–K12 na trasy, zasoby i testy, jeśli
      ...
      o ratyfikację).
- [ ] 6.2 Wpisać odpowiedzi strażnika **dosłownie** w kolumnę „Odpowiedź strażnika"
...
- [ ] 6.4 Odhaczyć w tym pliku zadania sekcji 2–5 z krótką notatką o tym, co faktycznie
      ...
      wyjaśnienia.
- [ ] 6.5 Bramki punktu kontrolnego: pełny `php artisan test`, `./vendor/bin/pint`,
      ...
      czyste.
- [ ] 6.6 Zsynchronizować gałąź z bazą: `git fetch origin && git rebase origin/main`;
      weryfikacja: `git log` zawiera aktualny `origin/main`, suita nadal zielona po
      rebase.
- [ ] 6.7 Zamknąć H08a osobnym commitem `feat(H08a): CMS kursów i lekcji` (`git add`
...
- [ ] 7.1 Sprawdzić w kontenerze `post_max_size` i `upload_max_filesize` — muszą być
...
- [ ] 7.2 Dopisać trasy materiałów do grupy administracyjnej:
...
- [ ] 7.3 Utworzyć `App\Http\Requests\H08\StoreMaterialRequest` — `file` wymagany,
...
- [ ] 7.4 Utworzyć `App\Services\H08\MaterialStore` — dysk `local`
      ...
      z H05).
- [ ] 7.5 Zaimplementować usuwanie materiału: **twarde** (tabela `materials` nie ma
...
- [ ] 7.6 Utworzyć `App\Http\Controllers\Api\V1\Admin\MaterialAdminController`
...
- [ ] 7.7 Utworzyć `frontend/components/h08b/CourseMaterialsPanel.tsx` i wpiąć go
...
- [ ] 7.8 Utworzyć `backend/tests/Feature/H08/MaterialUploadTest.php` ze
...
- [ ] 7.9 Bramki fazy: `--filter=H08`, pełna suita, `./vendor/bin/pint`,
...
- [ ] 8.1 Dopisać trasę `POST /admin/courses/{course}/invite` do grupy administracyjnej;
      weryfikacja: `route:list` pokazuje trasę pod `role:project_manager,super_admin`.
- [ ] 8.2 Utworzyć `App\Http\Requests\H08\InviteToCourseRequest` — `user_ids` wymagana
      tablica, `distinct`, każdy element `exists:users,id`; weryfikacja: testy walidacji.
- [ ] 8.3 Utworzyć `App\Http\Controllers\Api\V1\Admin\CourseInviteController` — dla
...
- [ ] 8.4 Zaimplementować regułę domenową: zapraszamy wyłącznie na kurs **poza główną
...
- [ ] 8.5 Utworzyć `frontend/components/h08b/CourseInvitePanel.tsx` i wpiąć dwiema
...
- [ ] 8.6 Utworzyć `backend/tests/Feature/H08/CourseInviteTest.php` — kryterium 5
...
- [ ] 8.7 Bramki fazy: `--filter=H08`, pełna suita, `./vendor/bin/pint`,
      `npm run lint -- --fix`, `npm run build`; weryfikacja: wszystkie zielone.
...
- [ ] 9.4 Odhaczyć pozostałe zadania sekcji 7–8 z notatkami o odstępstwach; weryfikacja:
...
- [ ] 9.6 Bramki końcowe: pełny `php artisan test`, `./vendor/bin/pint`,
...
- [ ] 9.7 Zacommitować część H08b jako `feat(H08b): materiały i zaproszenia` (H08a ma
...
- [ ] 9.11 Potwierdzić, że PR **nie został scalony przez autora** — merge należy do
